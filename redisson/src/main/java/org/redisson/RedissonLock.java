...
public class RedissonLock extends RedissonExpirable implements RLock {
    ...
    private void lock(long leaseTime, TimeUnit unit, boolean interruptibly) throws InterruptedException {
        ...
        // 加锁失败，则订阅到对应这个锁的channel，当其他线程释放锁时，通知线程去获取锁
...
//        get(lockAsync(leaseTime, unit));
    ...
    protected RFuture<Boolean> renewExpirationAsync(long threadId) {
         ...
         * ARGV[2](getLockName(threadId)) ：锁的唯一标识，也就是 id（UUID.randomUUID()） + “:” + threadId
         */
        return evalWriteAsync(getName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                // 若锁已存在，并且锁住的是当前线程，则重新设置过期时间
    ...
    <T> RFuture<T> tryLockInnerAsync(long leaseTime, TimeUnit unit, long threadId, RedisStrictCommand<T> command) {
        ...
        return evalWriteAsync(getName(), LongCodec.INSTANCE, command,
                ...
                "if (redis.call('exists', KEYS[1]) == 0) then " +
                        ...
                        "return nil; " +
                        "end; " +
                        // 如果锁已存在，并且锁的是当前线程，则通过hincrby给数值递增 1
                        "if (redis.call('hexists', KEYS[1], ARGV[2]) == 1) then " +
                        "redis.call('hincrby', KEYS[1], ARGV[2], 1); " +
                        // 锁冲入重新设置超时时间
                        "redis.call('pexpire', KEYS[1], ARGV[1]); " +
...
//        throw commandExecutor.convertException(future);
    ...
    protected RFuture<Boolean> unlockInnerAsync(long threadId) {
        ...
        return evalWriteAsync(getName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                // 如果锁已经存在，但不是当前线程获取的，则返回nil
                "if (redis.call('hexists', KEYS[1], ARGV[3]) == 0) then " +
                        "return nil;" +
                        "end; " +
                        // 由于支持可重入，因此获取锁的引用次数
                        "local counter = redis.call('hincrby', KEYS[1], ARGV[3], -1); " +
                        // 如果counter大于0，则重新设置过期时间，并返回0
                        "if (counter > 0) then " +
                        "redis.call('pexpire', KEYS[1], ARGV[2]); " +
                        "return 0; " +
                        "else " +
                        // 如果counter等于0，则删除key，并发布消息表示锁已经可用，并返回 1
